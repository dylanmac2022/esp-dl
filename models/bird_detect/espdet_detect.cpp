...
extern const uint8_t bird_detect_espdl[] asm("_binary_bird_detect_espdl_start");
static const char *path = (const char *)bird_detect_espdl;
...
{
    switch (model_type) {
    case model_type_t::ESPDET_PICO_224_224_BIRD:
...
void ESPDetDetect::load_model()
{
    switch (m_model_type) {
    case model_type_t::ESPDET_PICO_224_224_BIRD:
#if CONFIG_FLASH_ESPDET_PICO_224_224_BIRD || CONFIG_BIRD_DETECT_MODEL_IN_SDCARD
        m_model = new espdet_detect::ESPDet("espdet_pico_224_224_bird.espdl", m_score_thr[0], m_nms_thr[0]);
#else
        ESP_LOGE("bird_detect", "espdet_pico_224_224_bird is not selected in menuconfig.");
#endif
        break;
    }
}
